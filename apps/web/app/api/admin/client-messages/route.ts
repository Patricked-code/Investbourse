...
export async function GET() {
  ...
  const response = await fetch(`${API_BASE_URL}/api/office/client-messages`, { cache: "no-store" });
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/office/client-messages`, {
    ...
    body: JSON.stringify({ ...body, senderType: "ADMIN", senderLabel: session.user.fullName, actorUserId: session.user.id }),
  });
