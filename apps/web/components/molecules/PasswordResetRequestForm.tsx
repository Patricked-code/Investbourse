...
export function PasswordResetRequestForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDevToken(payload?.data?.devToken ?? null);
      setState("success");
    } catch (error) {
      setState("error");
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-xl shadow-slate-900/10">
      ...
      <p className="mt-3 leading-7 text-slate-600">Entrez votre email. Si le compte existe, un lien de réinitialisation temporaire sera envoyé par email.</p>
      ...
      {state === "success" ? <div className="mt-4 rounded-2xl bg-emerald-50 p-4 text-sm leading-7 text-emerald-800"><p className="font-semibold">Demande prise en compte. Vérifiez votre boîte email.</p>{devToken ? <p className="mt-2 break-all">Token développement : {devToken}</p> : null}</div> : null}
